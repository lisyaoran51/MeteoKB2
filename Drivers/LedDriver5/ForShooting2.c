...
// gcc ForShooting2.c -o ForShooting2


unsigned char** draw(int variant);
unsigned char** draw_motion(int variant, int sequence);
...
unsigned char** draw_motion(int variant, int sequence){
	...
	unsigned char** originalMatrix = draw(variant);
	...
	for (i = 0; i < width; i++) {
		int j;
		for (j = 0; j < height; j++) {
			matrix[i][j] = originalMatrix[i][j + sequence >= height ? j + sequence - height : j + sequence ];
