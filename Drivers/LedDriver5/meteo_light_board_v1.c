...
int init_module(void)
{
	...
	if (gpio_request(DECODE_A_PIN, "DECODE_A_PIN")) return -1;
	if (gpio_request(DECODE_B_PIN, "DECODE_B_PIN")) return -1;
	if (gpio_request(DECODE_C_PIN, "DECODE_C_PIN")) return -1;
	...
	gpio_direction_output(DECODE_A_PIN, 0);
	gpio_direction_output(DECODE_B_PIN, 0);
	gpio_direction_output(DECODE_C_PIN, 0);
...
void cleanup_module(void)
{
	...
	gpio_free(DECODE_A_PIN);
	gpio_free(DECODE_B_PIN);
	gpio_free(DECODE_C_PIN);
}
