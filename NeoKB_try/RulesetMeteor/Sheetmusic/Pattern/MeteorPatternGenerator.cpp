...
Pattern * MeteorPatternGenerator::generateNoteControlPoint(vector<Event*>* es, NoteControlPoint * note)
{
	...
	LOG(LogLevel::Debug) << "int MeteorSmConverter::Generate(vector<Event*>*, Event*) : Generate Fall at [" << (int)pitch << "][" << cloned->GetStartTime() << "], start time [" << cloned->GetStartTime() - fallTime << "], life time [" << fallTime << "].";
...
Pattern* MeteorPatternGenerator::generateRepeatPracticeEvent(vector<Event*>* es, SectionStartControlPoint * sectionStart)
{
	...
	LOG(LogLevel::Depricated) << "MeteorSmConverter::generateRepeatPracticeEvent() : generate repeat practice event on [" << cloned->GetSectionIndex() << "] section at [" << cloned->GetStartTime() + cloned->GetLifeTime() << "].";
	Pattern* pattern = new Pattern(cloned);
