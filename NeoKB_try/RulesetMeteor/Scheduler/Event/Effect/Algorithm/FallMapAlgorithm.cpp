...
int FallMapGenerateAlgorithm::ImplementGenerate(Map * m, EffectMapper<FallEffect>* em)
{
	...
	LOG(LogLevel::Depricated) << "FallMapGenerateAlgorithm::ImplementGenerate : meteorPos = " << meteorPos << ", current time = " << currentTime;
	// 公式: -256*y + 256 
	// 算流星燈每一個燈泡的亮度，從下面網上算
	...
	if(isAdded)
	LOG(LogLevel::Finer) << "FallMapGenerateAlgorithm::ImplementGenerate : current time = " << currentTime << [](int width, int height, Map* m) {
