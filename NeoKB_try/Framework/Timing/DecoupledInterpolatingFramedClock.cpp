...
int DecoupledInterpolatingFramedClock::ProcessFrame()
{
	...
	LOG(LogLevel::Depricated) << "DecoupledInterpolatingFramedClock::ProcessFrame() : determine if decoupled clock start. coupled = ["<< isCoupled << "], source running = [" << (sourceRunning ? 1 : 0) << "]";
	// ***讓分離的時鐘一直跟著現在的時間***
