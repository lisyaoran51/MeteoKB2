...
int SampleManager::RemoveSampleChannel(SoundBinding * soundBinding)
{
	...
	if (it != sampleChannelCache.end()) {
		...
		pendingActions.Add(this, [=]() {
			...
			LOG(LogLevel::Depricated) << "SampleManager::RemoveSampleChannel() : delete channel." << sampleChannel;
...
int SampleManager::ImportFromStable()
{
	...
	vector<string>* paths = stable->GetDirectories("SystemData/Resources/Samples");

	paths->push_back(stable->GetUsableDirectoryPathFor("SystemData/Resources/Samples"));
